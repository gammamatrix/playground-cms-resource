            ...
            @if (config('playground-cms-resource.revisions.optional'))
                ...
                <div class="form-check form-check-inline">
                    ...
                    <input class="form-check-input" type="checkbox" id="revision" name="revision" value="1"
                        {{ config('playground-cms-resource.revisions.pages') ? 'checked' : '' }}>
                    <label class="form-check-label" for="revision">Revision</label>
                    <p class="form-text text-muted">
                        @if (config('playground-cms-resource.revisions.pages'))
                            {{ __('playground-cms-resource::revisions.pages.enabled') }}
                        @else
            ...
            @else
                <p class="form-text text-muted">
                    @if (config('playground-cms-resource.revisions.pages'))
                        {{ __('playground-cms-resource::revisions.pages.enabled') }}
                    @else
                        {{ __('playground-cms-resource::revisions.pages.disabled') }}
                    @endif
                </p>
            @endif
